<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
      <el-form-item>
        <el-input v-model="dataForm.key" placeholder="參數名" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button @click="getDataList()">查詢</el-button>
        <el-button v-if="isAuth('hr:principal:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
      </el-form-item>
    </el-form>
    <el-table
            :data="dataList"
            border
            v-loading="dataListLoading"
            @selection-change="selectionChangeHandle"
            style="width: 100%;">
      <el-table-column
              type="selection"
              header-align="center"
              align="center"
              width="50">
      </el-table-column>
              <el-table-column
                prop="id"
                header-align="center"
                align="center"
                label="序號">
        </el-table-column>
              <el-table-column
                prop="employId"
                header-align="center"
                align="center"
                label="員工編號">
        </el-table-column>
              <el-table-column
                prop="name"
                header-align="center"
                align="center"
                label="姓名">
        </el-table-column>
              <el-table-column
                prop="dateOfEmployment"
                header-align="center"
                align="center"
                label="到職日期">
        </el-table-column>
              <el-table-column
                prop="resignationDate"
                header-align="center"
                align="center"
                label="離職日期">
        </el-table-column>
              <el-table-column
                prop="jobTitle"
                header-align="center"
                align="center"
                label="職稱">
        </el-table-column>
              <el-table-column
                prop="idNumber"
                header-align="center"
                align="center"
                label="身份證字號">
        </el-table-column>
              <el-table-column
                prop="birth"
                header-align="center"
                align="center"
                label="出生年月日">
        </el-table-column>
              <el-table-column
                prop="bloodType"
                header-align="center"
                align="center"
                label="血型">
        </el-table-column>
              <el-table-column
                prop="mobileNumber"
                header-align="center"
                align="center"
                label="行動電話">
        </el-table-column>
              <el-table-column
                prop="contactNumber"
                header-align="center"
                align="center"
                label="連絡電話">
        </el-table-column>
              <el-table-column
                prop="emergencyContact"
                header-align="center"
                align="center"
                label="緊急連絡人/關係">
        </el-table-column>
              <el-table-column
                prop="emergencyContactPhone"
                header-align="center"
                align="center"
                label="緊急連絡人電話">
        </el-table-column>
              <el-table-column
                prop="address"
                header-align="center"
                align="center"
                label="連絡地址">
        </el-table-column>
              <el-table-column
                prop="education"
                header-align="center"
                align="center"
                label="學歷">
        </el-table-column>
              <el-table-column
                prop="graduateSchoolDepartment"
                header-align="center"
                align="center"
                label="畢業學校/科系">
        </el-table-column>
              <el-table-column
                prop="probation"
                header-align="center"
                align="center"
                label="試用期滿日">
        </el-table-column>
              <el-table-column
                prop="seniority"
                header-align="center"
                align="center"
                label="年資">
        </el-table-column>
              <el-table-column
                prop="age"
                header-align="center"
                align="center"
                label="年齡">
        </el-table-column>
              <el-table-column
                prop="email"
                header-align="center"
                align="center"
                label="個人Email">
        </el-table-column>
            <el-table-column
              fixed="right"
              header-align="center"
              align="center"
              width="150"
              label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="addOrUpdateHandle(scope.row.id)">修改</el-button>
          <el-button type="text" size="small" @click="deleteHandle(scope.row.id)">刪除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
            @size-change="sizeChangeHandle"
            @current-change="currentChangeHandle"
            :current-page="pageIndex"
            :page-sizes="[10, 20, 50, 100]"
            :page-size="pageSize"
            :total="totalPage"
            layout="total, sizes, prev, pager, next, jumper">
    </el-pagination>
    <!-- 彈窗, 新增 / 修改 -->
    <add-or-update v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDataList"></add-or-update>
  </div>
</template>

<script>
  import AddOrUpdate from './principal-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
          key: ''
        },
        dataList: [],
        pageIndex: 1,
        pageSize: 10,
        totalPage: 0,
        dataListLoading: false,
        dataListSelections: [],
        addOrUpdateVisible: false
      }
    },
    components: {
      AddOrUpdate
    },
    activated () {
      this.getDataList()
    },
    methods: {
      // 獲取數據列表
      getDataList () {
        this.dataListLoading = true
        this.$http({
        url: this.$http.adornUrl('/hr/principal/list'),
                method: 'get',
                params: this.$http.adornParams({
                'page': this.pageIndex,
                'limit': this.pageSize,
                'key': this.dataForm.key
      })
      }).then(({data}) => {
          if (data && data.code === 0) {
            this.dataList = data.page.list
            this.totalPage = data.page.totalCount
          } else {
            this.dataList = []
            this.totalPage = 0
          }
          this.dataListLoading = false
        })
      },
      // 每頁數
      sizeChangeHandle (val) {
        this.pageSize = val
        this.pageIndex = 1
        this.getDataList()
      },
      // 當前頁
      currentChangeHandle (val) {
        this.pageIndex = val
        this.getDataList()
      },
      // 多選
      selectionChangeHandle (val) {
        this.dataListSelections = val
      },
      // 新增 / 修改
      addOrUpdateHandle (id) {
        this.addOrUpdateVisible = true
        this.$nextTick(() => {
                this.$refs.addOrUpdate.init(id)
        })
      },
      // 刪除
      deleteHandle (id) {
        var ids = id ? [id] : this.dataListSelections.map(item => {
          return item.id
        })
        this.$confirm(`確定對[id=${ids.join(',')}]進行[${id ? '刪除' : '批量刪除'}]操作?`, '提示', {
                confirmButtonText: '確定',
                cancelButtonText: '取消',
                type: 'warning'
      }).then(() => {
          this.$http({
                  url: this.$http.adornUrl('/hr/principal/delete'),
                  method: 'post',
                  data: this.$http.adornData(ids, false)
        }).then(({data}) => {
            if (data && data.code === 0) {
              this.$message({
                      message: '操作成功',
                      type: 'success',
                      duration: 1500,
                      onClose: () => {
                this.getDataList()
              }
            })
            } else {
              this.$message.error(data.msg)
            }
          })
        })
      }
    }
  }
</script>
