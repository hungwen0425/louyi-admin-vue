...
// 開發環境不使用懶加載, 因為懶加載頁面太多的話會造成 webpack 熱更新太慢, 所以只有生產環境使用懶加載
...
const mainRoutes = {
  ...
  children: [
    ...
    // 2. iframeUrl: 是否通過 iframe 嵌套展示內容, '以 http[s]://開頭': 是, '': 否
    // 提示: 如需要通過 iframe 嵌套展示內容, 但不通過 tab 打開, 請自行創建組件使用 iframe 處理!
...
router.beforeEach((to, from, next) => {
  ...
  if (router.options.isAddDynamicMenuRoutes || fnCurrentRouteType(to, globalRoutes) === 'global') {
    ...
    http({
      ...
      console.log(`%c${e} 請求選單列表和權限失敗，跳轉至登錄頁！！`, 'color:blue')
